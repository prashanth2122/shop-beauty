...
@Component({
  selector: 'app-oudh-accord-gold',
  templateUrl: './oudh-accord-gold.component.html',
  styleUrls: ['./oudh-accord-gold.component.scss'],
  encapsulation:ViewEncapsulation.None
})
export class OudhAccordGoldComponent implements OnInit {
  @ViewChild("oudhgold") oudhgold: ElementRef;
  param1: string;
  constructor(
   ...
   ngOnInit() {
    this.titleService.setTitle('Mesmerising Oudh & Gold | Behind the Fragrance | Molton Brown® UK');
  ...
  setLang(lng) {
    this.featureServ.getStaticContent(lng).subscribe(response => {
      ...
      const _data = response["feature"]['oudhgold'];
    ...
    const baseSite = this.singletonServ.catalogVersion;
      ...
      AmpCa.utils.getHtmlServiceData({
        ...
        callback: function(data) {
          if (data) {
            that.oudhgold.nativeElement.innerHTML = data;
          }
        }
      });
  }
