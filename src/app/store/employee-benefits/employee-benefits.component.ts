...
export class EmployeeBenefitsComponent implements OnInit {
  ...
  @ViewChild("employeBenefits") employeBenefits: ElementRef;
  param1: string;
  constructor(
  ...
  setLang(lng) {
    this.featureServ.getStaticContent(lng).subscribe(response => {
      ...
      const _data = response["store"]['employeeBenefits']['content'];
    ...
    const baseSite = this.singletonServ.catalogVersion;
      ...
      AmpCa.utils.getHtmlServiceData({
        ...
        callback: function(data) {
          if (data) {
            that.employeBenefits.nativeElement.innerHTML = data;
          }
        }
      });
  }
