...
@Component({
  selector: 'app-tobacco-absolute',
  templateUrl: './tobacco-absolute.component.html',
  styleUrls: ['./tobacco-absolute.component.scss'],
  encapsulation:ViewEncapsulation.None
})
export class TobaccoAbsoluteComponent implements OnInit {
 
  @ViewChild("tobaccoabsolute")  tobaccoabsolute: ElementRef;
  param1: string;
  constructor(
  ...
  setLang(lng) {
    this.featureServ.getStaticContent(lng).subscribe(response => {
      ...
      const _data = response["feature"]['tobaccoabsolute'];
    ...
    const baseSite = this.singletonServ.catalogVersion;
      ...
      AmpCa.utils.getHtmlServiceData({
        ...
        callback: function(data) {
          if (data) {
            that.tobaccoabsolute.nativeElement.innerHTML = data;
          }
        }
      });
  }
