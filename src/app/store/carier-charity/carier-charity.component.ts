...
@Component({
  selector: 'app-carier-charity',
  templateUrl: './carier-charity.component.html',
  styleUrls: ['./carier-charity.component.scss']
})
export class CarierCharityComponent implements OnInit {
 
  @ViewChild("charityEl") charityEl: ElementRef;
  param1: string;
  constructor(
  ...
  setLang(lng) {
    this.featureServ.getStaticContent(lng).subscribe(response => {
      ...
      const _data = response["store"]['charity']['content'];
    ...
    const baseSite = this.singletonServ.catalogVersion;
      ...
      AmpCa.utils.getHtmlServiceData({
        ...
        callback: function(data) {
          if (data) {
            that.charityEl.nativeElement.innerHTML = data;
          }
        }
      });
  }
