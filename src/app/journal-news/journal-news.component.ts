...
@Component({
  selector: 'app-journal-news',
  templateUrl: './journal-news.component.html',
  styleUrls: ['./journal-news.component.scss']
})
export class JournalNewsComponent implements OnInit {
  param1:any;
  constructor(
  ...
  setLang(lng) {
    this.featureServ.getStaticContent(lng).subscribe(response => {
      ...
      const _data = response['feature']['news'];
      _data.sort((a, b) => {
