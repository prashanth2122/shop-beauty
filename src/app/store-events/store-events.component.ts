import { Inject,Component,  ViewChild,OnInit, ElementRef,OnDestroy } from '@angular/core';
...
export class StoreEventsComponent implements OnInit {
  ...
  setLang(lng) {
    this.featureServ.getStaticContent(lng).subscribe(response => {
      ...
      const _data = response["feature"]['storeevents'];
    ...
    const baseSite = this.singletonServ.catalogVersion;
      ...
      AmpCa.utils.getHtmlServiceData({
        ...
        callback: function(data) {
          if (data) {
            that.storeEvents.nativeElement.innerHTML = data;
          }
        }
      });
  }
